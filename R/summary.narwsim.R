 ...
 # Preamble ---------------------------------------------------------------
  ...
  if("health" %in% what | what == "all"){
    ...
    if(nrow(n.dead.region)> 0){
      ...
      n.dead.cause <- dead.df[, .N, list(abb, cause_death)] |> 
        tidyr::pivot_wider(names_from = abb, values_from = N) |> 
        format_dt(direction = "col")
    ...
    if(4 %in% cohortID){
      ...
      cat("\n")
      cat("Abortion rate: ", 100 * round(as.numeric(obj$abort[whale %in% whaleID, .(abort = sum(abort))]) / nrow(obj$abort), 1), 
    ...
    if (5 %in% cohortID) {
      ...
      if(!is.null(obj$birth[[1]])){
        ...
        cat("No. births:", paste0(n.births, " (", round(100 * n.births / n.ind, 1), "%)\n"))
