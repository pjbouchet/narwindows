...
get_scenarios <- function(scenario = 1){
  ...
  vparams <- vparams[phase == ifelse(scenario < 3, "Construction", "Operation and maintenance")]
...
get_transits <- function(){
  ...
  vt <- readr::read_csv("data/vessels/vessel_transits.csv", col_types = c("fffiiiiii")) |>
    dplyr::rename(speed_knt = speed, width_m = width, length_m = length, Nvessels = Nv) |> 
    data.table::as.data.table()
  
  vt[, routeID:=paste0(windfarm, "-", route)]
    # tidyr::pivot_longer(cols = tidyr::starts_with("F"), names_to = "farm", values_to = "Ntrips") |>
    # dplyr::filter(Ntrips>0) |>
    # dplyr::select(-site) |>
    # dplyr::rename(site = farm) |>
    # dplyr::mutate(site = as.numeric(gsub(pattern = "F", replacement = "", site))) |>
    # dplyr::relocate(site, .before = "phase") |>
    # dplyr::relocate(Ntrips, .before = "width") |>
    # dplyr::rename(speed_knt = speed, width_m = width, length_m = length, Nvessels = Nv) |>
    # as.data.frame()
...
proxy_prey <- function(){
  ...
  out <- purrr::map(.x = 1:12, .f = ~{
    ...
    nc.gom <- rescale_raster(nc.gom, new.min = 200, new.max = max.N[.x])
    ...
    nc.gsl <- rescale_raster(nc.gsl, new.min = 200, new.max = max.N[.x])
...
plot_raster <- function(r, duke = FALSE, positive = FALSE, quantile = FALSE, breaks = NULL, title = NULL){
...
console <- function(msg, tick = FALSE){
  if(!tick){
    cat(paste0(msg, " ..."), sep = "")
  } else {
...
get_dates <- function(start.month = 10, strip = TRUE, gantt = FALSE){
  ...
  if(gantt){
    ...
    date_seq <- seq(start.date, by = 'day', length.out = 365)
    if(strip){
