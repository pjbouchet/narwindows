...
narw <- function(nsim = 1e3,
  ...
  # Default values
  if("cohortID" %in% names(args)) cohortID <- args[["cohortID"]] else cohortID <- 1:6
  ...
  # 
  # if(leap){
  #   date_seq <- date_seq[which(!grepl(pattern = "02-29", x = date_seq))]
  #   warning("Leap day not included.")
  # }
  ...
  preymaps <- lapply(dummy_prey, raster::as.matrix) # Monthly
  ...
  if(length(cohortID) > 1){
    ...
    console(paste0("Initializing parallel computing (", n.cores, " cores)"))
    ...
    console(paste0("Initializing parallel computing (", n.cores, " cores)"), TRUE)
 ...
 out.dt <- purrr::map(.x = out.t,
  ...
  # print(gam.dt)
  if(length(cohortID) > 1){
    console("Fitting terminal functions")
    cat("\n")
 ...
 # )
  ...
  console("Fitting terminal functions", TRUE)
